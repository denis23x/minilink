...
const ResponsiveDialogContext = createContext<{ isDesktop: boolean; open: boolean; onOpenChange: (open: boolean) => void }>({
  isDesktop: false,
  open: false,
  onOpenChange: () => void 0,
});
...
export function ResponsiveDialogTrigger({ children }: { children: React.ReactNode }) {
  return <>{children}</>;
}
...
export function ResponsiveDialogClose({ children }: { children: React.ReactNode }) {
  const { isDesktop } = useContext(ResponsiveDialogContext);
  if (isDesktop) return null;
